<template>
  <a :href="product.permalink || '#'" 
     class="group block bg-dark-800/50 rounded-2xl border border-dark-700 overflow-hidden hover:border-primary-500/30 hover:shadow-glow transition-all duration-300 hover:-translate-y-1">
    <!-- Image -->
    ...
    <div class="p-4">
      ...
      <p v-if="product.categories && product.categories.length > 0" 
         class="text-xs text-primary-400 font-medium mb-1.5 uppercase tracking-wide">
        {{ product.categories[0].name }}
      </p>

      <!-- Name -->
      <h3 class="text-sm font-semibold text-dark-100 group-hover:text-dark-50 transition-colors line-clamp-2 mb-2 leading-snug">
        {{ product.name }}
      </h3>

      <!-- Rating -->
      <div v-if="product.averageRating > 0" class="flex items-center gap-1.5 mb-2">
        <div class="flex items-center gap-0.5">
          <svg v-for="i in 5" :key="i" class="w-3.5 h-3.5" 
               :class="i <= Math.round(product.averageRating) ? 'text-accent-400' : 'text-dark-600'"
      ...
      <div class="flex items-center gap-2">
        ...
        <span v-if="product.onSale" class="text-sm text-dark-500 line-through">
