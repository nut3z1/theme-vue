    ...
    <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div v-else class="space-y-6">
        <div
          ...
          :key="cat.id"
          class="bg-dark-950 rounded-xl px-4 py-7"
        >
          <!-- Category Header -->
          <div class="flex items-center justify-between mb-5">
            <div class="flex items-center gap-3">
              <div
                class="w-1 h-6 bg-gradient-to-b from-primary-500 to-secondary-500 rounded-full"
              ></div>
              <h3 class="text-lg font-display font-bold text-dark-100">
                {{ cat.name }}
              </h3>
              <span
                class="text-xs text-dark-500 bg-dark-800 px-2 py-0.5 rounded-full"
...
onMounted(async () => {
  try {
    ...
    const catRes = await axios.get(
      `${restUrl}wc/store/v1/products/categories`,
      {
    ...
    // Fetch 5 products for each category (in parallel)
