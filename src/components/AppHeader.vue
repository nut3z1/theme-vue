    ...
    <header class="fixed top-0 left-0 right-0 z-50 bg-white shadow-sm">
        ...
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center gap-3 justify-between h-9">
            ...
            <span class="text-xs text-gray-500">
              Thời gian mở cửa: <strong class="text-gray-700 font-medium">Từ 8h – 18h hàng ngày</strong>
            </span>
      ...
      <div class="lg:hidden border-b border-gray-200 bg-white">
        <div class="flex items-center justify-between h-14 px-4">
          ...
          <!-- Center: Logo -->
          <a :href="homeUrl" class="absolute left-1/2 -translate-x-1/2 flex items-center gap-1.5">
            <img
              v-if="siteLogo"
              :src="siteLogo"
              :alt="siteTitle"
            ...
            />
            <span v-else class="text-lg font-bold text-gray-800">{{ siteTitle }}</span>
          </a>
          ...
          <div class="flex items-center gap-1">
            ...
            <InstantSearch>
              <template #trigger>
                <button class="p-2 text-gray-600 hover:text-blue-600 rounded-md transition-colors" aria-label="Tìm kiếm">
      ...
      <div class="hidden lg:block border-b border-gray-200">
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center gap-8 h-20">
            ...
            <!-- Logo -->
            <a :href="homeUrl" class="flex-shrink-0 flex items-center gap-2 group">
              <img
                v-if="siteLogo"
                :src="siteLogo"
                :alt="siteTitle"
              ...
              <div
                v-else
                class="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-600 to-blue-400 flex items-center justify-center"
              >
                <svg class="w-6 h-6 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/>
                </svg>
              </div>
              <span v-if="!siteLogo" class="text-lg font-bold text-gray-800">{{ siteTitle }}</span>
            </a>
            ...
            <div class="flex-1 max-w-2xl">
              <InstantSearch>
                <template #trigger>
                  <div class="flex items-center border border-gray-300 rounded-md overflow-hidden hover:border-blue-400 transition-colors cursor-pointer bg-white">
                    <div class="flex items-center px-3 py-2.5 flex-1 gap-2 text-gray-400">
            ...
            <div class="flex items-center gap-8 flex-shrink-0">
              ...
              <!-- Hotline -->
              <a href="tel:0975388726" class="flex items-center gap-2 text-gray-600 group  transition-colors">
                ...
                <div class="leading-tight">
                  ...
                  <div class="text-sm font-semibold text-gray-700 group-hover:text-blue-600">0975 388 726</div>
                </div>
              </a>

              <!-- Account -->
              <a :href="accountUrl" class="flex flex-col items-center gap-0.5 text-gray-600 hover:text-blue-600 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="hidden lg:block border-b border-gray-200 bg-white">
        <div class="max-w-8xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center gap-0 h-10 overflow-x-auto scrollbar-none">
        ...
        >
          <nav class="max-w-8xl mx-auto px-4 py-3 space-y-1 h-[100vh]">
            ...
            <div class="pt-2 border-t border-gray-100">
              <p class="px-4 py-1 text-xs font-semibold text-gray-400 uppercase tracking-wide">Danh mục</p>
              <a
                v-for="cat in navCategories"
                :key="`m-${cat.id}`"
                :href="cat.permalink"
...
function isCategoryActive(slug) {
  const path = window.location.pathname;
  if (wpData.currentCategory && wpData.currentCategory.slug === slug) return true;
  return path.includes(`/danh-muc-san-pham/${slug}`) || path.includes(`/product-category/${slug}`);
}
