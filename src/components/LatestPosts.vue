      ...
      <div class="bg-dark-950 rounded-xl px-4 py-7">
        ...
        >
          <div
            v-for="i in 5"
            :key="i"
            class="bg-dark-800/50 rounded-2xl border border-dark-700 overflow-hidden animate-pulse"
          >
        ...
        >
          ...
          >
            <div class="relative h-full">
              <div
                class="aspect-[4/3] lg:aspect-auto lg:h-full overflow-hidden bg-dark-800"
              >
                <img
                  v-if="posts[0].featuredImage"
                ...
                <div
                  v-else
                  class="w-full h-full min-h-[200px] flex items-center justify-center bg-gradient-to-br from-primary-500/10 to-secondary-500/10"
                >
                  <svg
              ...
              <div class="absolute bottom-0 left-0 right-0 p-5 lg:p-6">
                <div v-if="posts[0].categories?.length" class="mb-2">
                  <span
                    class="px-2.5 py-0.5 text-xs font-semibold text-primary-300 bg-primary-500/20 backdrop-blur-sm rounded-full"
                  >
                    {{ posts[0].categories[0].name }}
                  </span>
                </div>
                <h3
                  class="text-lg lg:text-xl font-display font-bold text-dark-50 group-hover:text-primary-300 transition-colors line-clamp-2 mb-2"
                >
                  {{ posts[0].title }}
                </h3>
                <p class="text-sm text-dark-300 line-clamp-2 hidden lg:block">
                  {{ posts[0].excerpt }}
                </p>
          ...
          >
            <!-- Image -->
            <div
              class="relative w-28 sm:w-32 lg:w-full shrink-0 overflow-hidden bg-dark-800"
            >
              <div class="aspect-square lg:aspect-[4/3]">
                ...
                <div
                  v-else
                  class="w-full h-full flex items-center justify-center bg-gradient-to-br from-primary-500/5 to-secondary-500/5"
                >
                  <svg
                    class="w-8 h-8 text-dark-600"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            <div class="flex-1 p-3 lg:p-4 flex flex-col justify-center">
              <time class="text-[10px] text-dark-500 mb-1">{{
                post.dateFormatted
              }}</time>
              <h3
                class="text-sm font-semibold text-dark-100 group-hover:text-primary-400 transition-colors line-clamp-2 leading-snug"
              >
                {{ post.title }}
              </h3>
            </div>
        ...
        <div class="sm:hidden text-center mt-6">
          <a
            :href="blogUrl"
            class="inline-flex items-center gap-1.5 text-sm font-medium text-primary-400"
          >
            Xem tất cả bài viết →
          </a>
        </div>
...
onMounted(async () => {
  try {
    const params = { per_page: 7, orderby: "date", order: "desc", _embed: true };
    if (excludeCatId) {
      params.categories_exclude = excludeCatId;
    }
    const res = await axios.get(`${restUrl}wp/v2/posts`, { params });
